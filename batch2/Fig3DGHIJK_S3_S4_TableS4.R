...
library(data.table);library(openxlsx);library(ggpubr)
sheets(loadWorkbook('output/batch2/processed_data.xlsx'))
dat.list<-readRDS('work/batch2/cleaned_data_refined.rds')
dat.wNA<-read.xlsx('output/batch2/processed_data.xlsx',sheet=4,check.names = F)
colnames(dat.wNA)<-gsub('.',' ',colnames(dat.wNA),fixed = T)
# hist(colSums(is.na(dat.wNA)))
dat.list<-readRDS('work/batch2/cleaned_data_refined.rds')
metab<-dat.list$data
...
saveRDS(ori.col,'work/batch2/ori_col.rds')
...
pdf('output/batch2/Fig3D_var_exp.pdf',w=5,h=2)
...
pdf('output/batch2/Fig3G_cor_vs_var_exp.pdf',w=5.1,h=2.5)
...
dev.off()
pdf('output/batch2/FigS4_cor_vs_var_exp_full.pdf',w=12,h=15)
ggplot(cor.stat,aes(x=variance,y=estimate))+
...
fwrite(cor.stat,'output/batch2/TableS4_passage_cor_and_origin_var.csv')
plot.m<-function(m){
...
pdf('output/batch2/Fig3HIJK_select_passage_fidelity.pdf',w=10,h=4.8)
...
# metab.sub<-metab[,intersect(as.character(multi.var.df[variable=='Origin'][value>.5][['metabolite']]),as.character(cor.stat[estimate>.2][['metabolite']]))]
metab.sub[]<-apply(metab.sub,2,scale)
# order cluster
pdf('output/batch2/FigS3_All_passage_characteristic_metabolite_heatmap.pdf',w=11,h=8.2)
...
draw(Heatmap(t(metab.sub),show_column_names = F,cluster_columns = dd.reorder,
             ...
             right_annotation = HeatmapAnnotation(which = 'row',`P0 P6 correlation`=cor.stat[match(colnames(metab.sub),metabolite)][['estimate']],simple_anno_size = unit(.2, "cm"),
