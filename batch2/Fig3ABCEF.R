...
dat.list<-readRDS('work/batch2/cleaned_data_refined.rds')
metab<-dat.list$data
...
fwrite(pv_df.mxLinear_passage,'output/batch2/TableS3_stats_compare_PDX_by_passage_mixed_effect_linear_with_imputed_data.csv')
metab.ref<-fread('work/batch2/metab_ref.csv')
...
pdf('output/batch2/Fig3A_pw_anal_pssg_main_class.pdf',w=4,h=2.8)
ggplot(stat.df.1[lib=='main_class'&!is.na(pv)&!is.na(enrichment.ratio)],aes(x=enrichment.ratio,y=-log10(pv),size=enrichment.ratio,color=change,label=feature))+
  ...
  theme_bw()+scale_color_manual(values = structure(rev(brewer.pal(7,'Reds'))[c(5,2)],names=c('decrease','increase')))+
...
ggplot(stat.df.1[lib=='kegg_pathway'&!is.na(pv)&!is.na(enrichment.ratio)],aes(x=enrichment.ratio,y=-log10(pv),size=enrichment.ratio,color=change,label=feature))+geom_point(alpha=.3)+
  geom_text_repel(stat.df.1[lib=='kegg_pathway'&!is.na(pv)&!is.na(enrichment.ratio)][pv<.1],mapping=aes(x=enrichment.ratio,y=-log10(pv),label=feature))+theme_bw()+
  scale_color_manual(values = structure(rev(brewer.pal(7,'Reds'))[c(5,2)],names=c('decrease','increase')))+
  theme(legend.position = 'bottom',legend.direction = 'horizontal',legend.background = element_blank())+labs(color='change with passage')+
  guides(size = "none")+ggtitle("Pathway analaysis by KEGG pathway")
dev.off()
##### Make Heatmaps ##### 
...
mk.hm(Glycerophosphocholines.dat)
...
mk.hm(Glycerophosphoethanolamines.dat)



pdf('output/batch2/Fig3C_heatmap_Pssg_composite.pdf',w=4.6,h=4.4)
...
pdf('output/batch2/Fig3E_PCA_all.pdf',w=4,h=3)
...
dev.off()
pdf('output/batch2/Fig3F_PCA_by_pt.pdf',w=4,h=3)
ggplot(pc.df,aes(x=PC1,y=PC2,color=Tumor.Name,alpha=Passage,size=Passage))+geom_point()+theme_bw()+
  scale_color_manual(values=ori.col[-length(ori.col)])+
  scale_alpha_manual(values=structure(seq(from = 0.1, to = 1, length.out = 7),names=paste0('P',0:6)))+
  scale_size_manual(values=structure(seq(from = 5, to = 1, length.out = 7),names=paste0('P',0:6)))+
