...
saveWorkbook(wb,'2024_paper/output/batch1/processed_data.xlsx',overwrite = T)
################################################################################################
################################################################################################
################################################################################################
...
pdf('2024_paper/output/batch1/Fig2AB_univ_multi_side_by_side.pdf',w=11.6,h=2);g1+g3+plot_layout(design = "ABB");dev.off()
...
pdf('2024_paper/output/batch1/Fig2B_multivar_legend.pdf',w=3.3,h=1)
...
dev.off()
# save results from this multivariate model
saveRDS(sample.stat,'2024_paper/work/batch1/multi_sample_stat.rds')
library(openxlsx);source('/project/CRI/DeBerardinis_lab/lcai/CommonFunctions/beautify_df.R')
wb<-createWorkbook()
...
saveWorkbook(wb,'2024_paper/output/batch1/multivar_stat.xlsx',overwrite = T)
...
pdf('2024_paper/output/batch1/Fig2C_volcano_sample_type.pdf',w=4.2,h=4.2)
...
dev.off()
grep("phthoa|Methylglut|Guanidino|eoph|ethylx|reatinine|Urate|Allantoin",rownames(metab),value = T)
...
library(ggpubr)
pdf('2024_paper/output/batch1/Fig2D_boxplot_PDX_vs_Human_1.pdf',w=6.5,h=2.1);plot.select(1);dev.off()
pdf('2024_paper/output/batch1/Fig2F_boxplot_PDX_vs_Human_2.pdf',w=4.4,h=2.1);plot.select(2);dev.off()
pdf('2024_paper/output/batch1/Fig2E_boxplot_PDX_vs_Human_3.pdf',w=2.3,h=2.1);plot.select(3);dev.off()
pdf('2024_paper/output/batch1/Fig2G_boxplot_PDX_vs_Human_4.pdf',w=4.4,h=2.1);plot.select(4);dev.off()
...
dev.off()
pdf('2024_paper/output/batch1/Fig2J_distance_compare_boxplot_original.pdf',h=2.1,w=1.5);comp.dist(t(metab));dev.off()
